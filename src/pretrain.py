"""Pre-training script for Vision-Language Model.

This module provides the main entry point for pre-training the VLM model
with command-line argument parsing and training orchestration.
"""
...
from src.schema import VLMModelConfig, VLMTrainerConfig
...
def parse_args():
    """
    Parse command-line arguments for VLM pretraining.

    Returns:
        argparse.Namespace: Parsed arguments containing model configuration,
            training parameters, and data paths.
    """
    parser = argparse.ArgumentParser(description="VLM Pretraining")
    ...
    parser.add_argument(
        "--vision_model",
        type=str,
        default="models/siglip-base-patch16-256-multilingual",
        help="Vision model path",
    )
    parser.add_argument(
        "--num_image_tokens", type=int, default=64, help="Number of image tokens"
    )
    parser.add_argument(
        "--attn_implementation",
        type=str,
...
def main():
    ...
    tokenizer, language_model = load_tokenizer_and_model(model_config)
    vision_processor, vision_model = load_vision_processor_and_model(model_config)
